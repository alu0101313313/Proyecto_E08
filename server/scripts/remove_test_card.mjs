...
const run = async () => {
  ...
  if (!card) {
    console.log('No se encontró carta de prueba. Nada que borrar.');
    await mongoose.disconnect();
    process.exit(0);
  }

  const cardId = card._id;
  console.log('Encontrada carta de prueba _id =', cardId.toString());
  ...
  console.log('Usuarios actualizados (pull):', update.modifiedCount);
